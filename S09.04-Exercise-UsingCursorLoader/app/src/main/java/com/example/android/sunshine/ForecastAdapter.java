...
class ForecastAdapter extends RecyclerView.Adapter<ForecastAdapter.ForecastAdapterViewHolder> {
    ...
    public void onBindViewHolder(ForecastAdapterViewHolder forecastAdapterViewHolder, int position) {
        ...
        if (mCursor.move(position)) {
            int indexDate = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_DATE);
            int indexId = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID);
            int indexHigh = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP);
            int indexLow = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP);

            int dateInt = mCursor.getInt(indexDate);
            String dateString = SunshineDateUtils.getFriendlyDateString(mContext, dateInt, false);
            String idString = mCursor.getString(indexId);
            double highTemp = mCursor.getDouble(indexHigh);
            double lowTemp = mCursor.getDouble(indexLow);

            String highAndLowTemp =
                    SunshineWeatherUtils.formatHighLows(mContext, highTemp, lowTemp);

            // COMPLETED (7) Generate a weather summary with the date, description, high and low
            String weatherSummary = dateString + " - " + idString + " - " + highAndLowTemp;
